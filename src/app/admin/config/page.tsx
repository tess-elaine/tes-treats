        ...
        <form action={saveConfigAction} className="space-y-8">
          ...
          <section>
            ...
            <label className="mt-3 flex items-center gap-3">
              <input type="checkbox" name="pickupEnabled" defaultChecked={cfg?.pickupEnabled ?? true} className="h-4 w-4 accent-primary" />
              <span>Allow pickup at the address above</span>
            </label>
            <label className="mt-4 block">
              <span className="font-label uppercase tracking-[0.12em] text-on-surface-variant">Pickup instructions</span>
              <textarea
          ...
          <section>
            ...
            <label className="mt-3 flex items-center gap-3">
              <input type="checkbox" name="deliveryEnabled" defaultChecked={cfg?.deliveryEnabled ?? true} className="h-4 w-4 accent-primary" />
              <span>Offer local delivery</span>
            </label>
            <label className="mt-4 block">
              <span className="font-label uppercase tracking-[0.12em] text-on-surface-variant">Delivery zones</span>
              <p className="text-xs text-on-surface-variant">
                One zone per line: <code>Label | comma-separated zips | fee_in_cents</code>
              </p>
              <textarea
                name="zones"
                rows={6}
          ...
          <section>
            ...
            <label className="mt-3 flex items-center gap-3">
              <input type="checkbox" name="taxEnabled" defaultChecked={cfg?.taxEnabled ?? false} className="h-4 w-4 accent-primary" />
              <span>Collect sales tax (NY: most cookies sold whole are exempt — only enable when your accountant says so)</span>
            </label>
            <Field
          ...
          <BiteButton size="lg">Save site config</BiteButton>
        </form>
      </NibbleCard>
    </div>
  );
